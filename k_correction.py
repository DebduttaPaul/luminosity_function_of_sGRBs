...
#~ 
#~ 
#~ k_table	=	Table( [z_sim, dL_sim, k_BATSE_sim, k_Fermi_sim, k_Swift_sim, k_CZTI_sim, term_BATSE, term_Fermi, term_Swift], names = ['z', 'dL', 'k_BATSE', 'k_Fermi', 'k_Swift', 'k_CZTI', 'term_BATSE', 'term_Fermi', 'term_Swift'] )
#~ ascii.write( k_table, './../tables/k_correction.txt', format = 'fixed_width', overwrite = True )
#~ 
#~ 
#~ 
#~ ####################################################################################################################################################
...
ax.plot( z_sim, np.ones(z_sim.size), 'k--', ms = marker_size )
plt.savefig( './../plots/k_correction--Fermi.png' )
plt.savefig( './../plots/k_correction--Fermi.pdf' )
plt.clf()
plt.close()
...
ax.set_yscale('log')
ax.set_xlabel( r'$ z $', fontsize = size_font+2 )
ax.set_ylabel( r'$ \rm{ k \; [MeV] } $', fontsize = size_font, labelpad = padding-8 )
ax.plot( z_sim, k_Swift*1e-3, 'k-', ms = marker_size )
ax.plot( z_sim, np.ones(z_sim.size), 'k--', ms = marker_size )
plt.savefig( './../plots/k_correction--Swift.png' )
plt.savefig( './../plots/k_correction--Swift.pdf' )
plt.clf()
plt.close()
...
ax.set_yscale('log')
ax.set_xlabel( r'$ z $', fontsize = size_font+2 )
ax.set_ylabel( r'$ \rm{ k \; [MeV] } $', fontsize = size_font, labelpad = padding-6 )
ax.plot( z_sim, k_BATSE*1e-3, 'y-' , ms = marker_size, label = r'$ BATSE $' )
ax.plot( z_sim, k_Swift*1e-3, 'b--', ms = marker_size, label = r'$ Swift $' )
ax.plot( z_sim, k_CZTI *1e-3, 'g--', ms = marker_size, label = r'$ CZTI  $' )
