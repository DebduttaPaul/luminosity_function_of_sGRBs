...
from scipy.stats import pearsonr as R, spearmanr as S, kendalltau as T
...
Swift_wkr_fluence			=	Swift_wkr_data['BAT Fluence (15-150 keV) [10^-7 erg/cm^2]'].data										# in 10^{-7} erg.cm^{-2}.
Swift_wkr_fluence_error		=	Swift_wkr_data['BAT Fluence 90% Error (15-150 keV) [10^-7 erg/cm^2]'].data * (0.68/0.90)				# in 10^{-7} erg.cm^{-2}.
...
Swift_GRBs_table	=	Table( [ Swift_wkr_GRB_name, Swift_wkr_GRB_ID, Swift_wkr_Tt, Swift_wkr_RA, Swift_wkr_Dec, Swift_wkr_error_radius, Swift_wkr_T90, Swift_wkr_known_redshift, Swift_wkr_phoflux, Swift_wkr_phoflux_error, Swift_wkr_fluence, Swift_wkr_fluence_error ],
								names = [ 'Swift name', 'Swift ID', 'BAT Trigger-time', 'BAT RA', 'BAT Dec', 'BAT Error-radius', 'BAT T90', 'redshift', 'BAT Phoflux', 'BAT Phoflux_error', 'BAT fluence', 'BAT fluence_error' ] )
...
ax.set_ylabel( r'$ Fermi \; \, \rm{ T_{90} \; [sec] } $', fontsize = size_font )
ax.errorbar( common_redshift, common_Fermi_T90, yerr =  common_Fermi_T90_error, fmt = '.', ms = marker_size, color = 'silver', markerfacecolor = 'k', markeredgecolor = 'k' )
plt.savefig( './../plots/T90_Fermi_vs_z--common_GRBS--wkr.png' )
plt.clf()
plt.close()
...
ax.set_yscale('log')
ax.set_xlabel( r'$ \rm{ redshift } $', fontsize = size_font )
ax.set_ylabel( r'$ Swift \; \, \rm{ T_{90} \; [sec] } $', fontsize = size_font )
ax.errorbar( common_redshift, common_Swift_T90, fmt = '.', ms = marker_size, color = 'silver', markerfacecolor = 'k', markeredgecolor = 'k' )
plt.savefig( './../plots/T90_Swift_vs_z--common_GRBS--wkr.png' )
plt.clf()
plt.close()


print '\n\nCorrelations...'
print R( common_redshift, common_Fermi_T90 )
print S( common_redshift, common_Fermi_T90 )
print T( common_redshift, common_Fermi_T90 )
print R( common_redshift, common_Swift_T90 )
print S( common_redshift, common_Swift_T90 )
print T( common_redshift, common_Swift_T90 )
print '\n\n\n\n'
...
common_GRBs_table	=	Table( [ common_ID, common_Swift_name, common_Fermi_name, common_Swift_Tt, common_Fermi_Tt, common_Swift_RA, common_Fermi_RA, common_Swift_Dec, common_Fermi_Dec,
								 common_Swift_T90, common_Fermi_T90, common_Fermi_T90_error, common_redshift, common_Fermi_flux, common_Fermi_flux_error, common_Fermi_fluence, common_Fermi_fluence_error,
								 ...
								 names = [ 'common ID', 'Swift name', 'Fermi name', 'BAT Trigger-time', 'GBM Trigger-time', 'BAT RA', 'GBM RA', 'BAT Dec', 'GBM Dec', 
											'BAT T90', 'GBM T90', 'GBM T90_error', 'redshift', 'GBM flux', 'GBM flux_error', 'GBM fluence', 'GBM fluence_error',
